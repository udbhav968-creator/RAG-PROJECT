...
def run_benchmark():
    logger.info("Starting Industrial RAG Engine Benchmark...")
    queries = [
    ...
    for q in queries:
        ...
        logger.info(f"Query: '{q[:30]}...' -> {elapsed}s ({len(res.get('attempts', []))} attempts)")
        
    total_elapsed = round(time.time() - start_time, 3)
    avg_latency = round(total_elapsed / len(queries), 3)
    logger.info(f"Benchmark Complete. Total time: {total_elapsed}s | Avg Latency: {avg_latency}s/query")
